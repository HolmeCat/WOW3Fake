...
public:
    ...
    void WeaponSort(int n){ //武器排序
        if(!Take.empty()){ //STL算法Sort排序，自定义一个比较函数
            if(n == 3)
                sort(Take.begin(), Take.end(), comp3);
            if(n == 4)
                sort(Take.begin(), Take.end(), comp4);
        }
        return;
    }
    void report(){ //报告武器情况
...
public:
    ...
    void Process(){ //事件过程
        while(true){ //时间到或者任意司令部被占领就退出循环
            Minutes = 0;
            if(TimeJudge()) //在每个整点，即每个小时的第0分， 双方的司令部中各有一个武士降生。
                break; 
            ...
            for(int i = 0; i < N + 2; i++){ //在每个小时的第10分：所有的武士朝敌人司令部方向前进一步。
                if(i == 0){ //蓝方去红方司令部
                    ...
                    if(BG[i + 1]->Blue != NULL){
                        ...
                        cout << "blue ";
                        BG[i]->Blue->name();
                        cout << " " << BG[i]->Blue->ID << " reached red headquarter ";
                ...
                if(i == N + 1){ //红方去蓝方司令部
                    if(R != NULL){
                        ...
                        cout << "red ";
                        BG[i]->Red->name();
                        cout << " " << BG[i]->Red->ID << " reached blue headquarter ";
                ...
                if(BG[i]->Red != NULL){
                    ...
                    cout << "red ";
                    BG[i]->Red->name();
                    cout << " " << BG[i]->Red->ID << " marched to city " << i;
                    cout << " with " << BG[i]->Red->HP << " elements and force " << BG[i]->Red->ATK << endl;
                }
                if(BG[i + 1]->Blue != NULL){ //蓝方从东
                    ...
                    cout << "blue ";
                    BG[i]->Blue->name();
                    cout << " " << BG[i]->Blue->ID << " marched to city " << i;
                    cout << " with " << BG[i]->Blue->HP << " elements and force " << BG[i]->Blue->ATK << endl;
                }
            }
            if(Red->Blue != NULL || Blue->Red != NULL) //此处是占领&结束判断
                break;
            Minutes = 35; 
            if(TimeJudge())
                break;
            ...
            Time();
            cout << WHPElement << " elements in red headquarter" << endl;
            Time();
            cout << EHPElement << " elements in blue headquarter" << endl;
            Minutes = 55;
            if(TimeJudge())
                break;
